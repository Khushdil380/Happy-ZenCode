...
export interface EditorPatchGeneratorConfig extends PatchGeneratorConfig {
    /** Whether to use front (above code) or back (behind code) rendering */
    useFront?: boolean;
    /** Interval for automatic image switching (in seconds, 0 = disabled) */
    interval?: number;
    /** Random image order */
    random?: boolean;
}
 ...
 */
export class EditorPatchGenerator extends BasePatchGenerator<EditorPatchGeneratorConfig> {
    private readonly cssPlaceholder = '--happy-zencode-editor-placeholder';

    constructor(config: EditorPatchGeneratorConfig) {
        super({
            useFront: true,
            interval: 0,
            random: false,
     ...
     */
    private getStaticScript(): string {
     ...
     */
    private getDynamicScript(): string {
     ...
     */
    public static createThemeConfig(
        themeName: string,
        images: string[],
        customOptions?: Partial<EditorPatchGeneratorConfig>
    ): EditorPatchGeneratorConfig {
        const baseConfig: EditorPatchGeneratorConfig = {
            images,
            useFront: true,
            opacity: 0.1,
            size: 'cover',
