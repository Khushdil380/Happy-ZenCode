...
import { ThemeManager } from './theme/ThemeManagerNew';
...
export async function activate(context: vscode.ExtensionContext) {
    try {
        ...
        Utils.debugLog('Extension activation started', {
        ...
        context.subscriptions.push(
            ...
            vscode.commands.registerCommand('happy-zencode.showStatus', async () => {
                ...
                vscode.window.showInformationMessage(
                    `Happy Zencode Status:\\n${message}`,
                    ...(status.needsRestart ? ['Restart Now'] : [])
                ).then(selection => {
            ...
            vscode.commands.registerCommand('happy-zencode.removeImage', async () => {
                ...
                // Show quick pick with current images
                ...
                const imageOptions: ImageOption[] = currentImages.map((img: string, index: number) => ({
                    label: `Image ${index + 1}`,
                ...
                if (selectedImage) {
                    const updatedImages = currentImages.filter((_: any, index: number) => index !== selectedImage.imageIndex);
                    ...
                    vscode.window.showInformationMessage(
                        `Image removed. Remaining images: ${updatedImages.length}`,
                        'Apply Changes'
                    ).then(selection => {
        ...
        if (!hasShownWelcome) {
            vscode.window.showInformationMessage(
                'Welcome to Happy Zencode! Configure your background images in VS Code settings under "Background".',
                'Open Settings'
            ).then(selection => {
        ...
        vscode.window.showErrorMessage(
            `Happy Zencode activation failed: ${error}. Check the console for details.`
        );
    }
}

export function deactivate() {
    Utils.debugLog('Extension deactivating');
