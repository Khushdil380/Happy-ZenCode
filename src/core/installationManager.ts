...
import { CSSInjectionManager } from './cssInjectionManager';
...
export class InstallationManager {
    ...
    private cssInjectionManager: CSSInjectionManager;
    ...
    constructor(context: vscode.ExtensionContext, themeManager: ThemeManager, backgroundManager: BackgroundManager) {
        ...
        this.cssInjectionManager = new CSSInjectionManager(context);
    }
    ...
    async install(): Promise<void> {
        try {
            ...
            const backgrounds = config.get<any>('customBackgrounds', {});
            const opacity = config.get<any>('opacity', {});
            ...
            if (paths) {
                try {
                    ...
                    vscode.window.showInformationMessage(
                        'Happy Zencode theme installed successfully via file modification! Please restart VS Code to see changes.',
                        'Restart Now'
                    ).then(selection => {
     ...
     */
    private showManualInstallationInstructions(backgrounds: any, opacity: any): void {
        ...
        fs.promises.mkdir(path.dirname(cssPath), { recursive: true })
            ...
            .then(() => {
                vscode.window.showInformationMessage(
                    ...
                    'Show Instructions', 'Open CSS File'
                ).then(selection => {
     ...
     */
    private async installWithSudo(): Promise<void> {
        try {
            const paths = this.getVSCodePaths();
            if (!paths) {
            ...
            vscode.window.showInformationMessage(
                'Administrator installation is not yet implemented. Please try the CSS injection method instead.',
                'Try CSS Injection'
            ).then(selection => {
     ...
     */
    private async writeFiles(paths: { workbench: string; product: string }, css: string): Promise<void> {
     ...
     */
    private injectJavaScript(content: string, css: string): string {
...
/* ${identifier}-end */`;
    ...
    private generateCSS(): string {
        ...
        const customBackgrounds = config.get<any>('customBackgrounds', {});
        const opacity = config.get<any>('opacity', {});
        const enabled = config.get<boolean>('enabled', true);
        const style = config.get<any>('style', {
            "content": "''",
            "pointer-events": "none",
            "position": "absolute",
            "z-index": "99999",
...
.welcome-view::before {
     ...
     */
    private processImagePath(imagePath: string): string | null {
     ...
     */
    private async installViaCSSInjection(): Promise<void> {
        const css = this.generateCSS();
        ...
        if (customCSSConfig.has('imports')) {
            ...
            const existingImports = customCSSConfig.get('imports', []) as string[];
            
            // Create a temporary CSS file
            const tempCSSPath = path.join(this.context.globalStorageUri.fsPath, 'happy-zencode-styles.css');
            
            try {
                await fsPromises.mkdir(path.dirname(tempCSSPath), { recursive: true });
                ...
                vscode.window.showInformationMessage(
                    'Happy Zencode theme installed via Custom CSS extension! Please run "Reload Custom CSS" command.',
                    'Reload Custom CSS'
                ).then(selection => {
            ...
            vscode.window.showInformationMessage(
                'Could not modify VS Code files directly. For full theme support, please install the "Custom CSS and JS Loader" extension.',
                'Install Extension',
                'Apply Colors Only'
            ).then(selection => {
     ...
     */
    private async applyColorCustomizations(): Promise<void> {
        ...
        vscode.window.showInformationMessage(
            'Happy Zencode color theme applied! Background images require VS Code file modification or Custom CSS extension for full functionality.'
        );
    }
     ...
     */
    private generateManualCSS(backgrounds: any, opacity: any): string {
     ...
     * Show manual installation instructions
     */
    private showManualInstructions(cssPath: string): void {
        const instructions = `
